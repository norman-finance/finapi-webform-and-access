...
class DirectDebitWithAccountDetails(BaseModel):
    """
    Parameters for a direct debit with an account
    """ # noqa: E501
    orders: Annotated[List[DirectDebitOrder], Field(min_length=1, max_length=15000)] = Field(description="Direct debit orders")
    execution_date: date = Field(description="Execution date for the direct debit(s), in the format <code>YYYY-MM-DD</code>. May not be in the past.", alias="executionDate")
    ...
    receiver: DirectDebitWithAccountReceiver
    direct_debit_type: StrictStr = Field(description="Type of the direct debit; either <code>BASIC</code> or <code>B2B</code> (Business-To-Business).", alias="directDebitType")
    sequence_type: StrictStr = Field(description="Sequence type of the direct debit. Possible values:<br/>&bull; <code>OOFF</code> - means that this is a one-time direct debit order;<br/>&bull; <code>FRST</code> - means that this is the first in a row of multiple direct debit orders;<br/>&bull; <code>RCUR</code> - means that this is one (but not the first or final) within a row of multiple direct debit orders;<br/>&bull; <code>FNAL</code> - means that this is the final in a row of multiple direct debit orders.", alias="sequenceType")
    __properties: ClassVar[List[str]] = ["orders", "executionDate", "batchBookingPreferred", "profileId", "redirectUrl", "callbacks", "receiver", "directDebitType", "sequenceType"]
    ...
    @field_validator('direct_debit_type')
    def direct_debit_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['B2B', 'BASIC']):
            raise ValueError("must be one of enum values ('B2B', 'BASIC')")
        return value

    @field_validator('sequence_type')
    def sequence_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['OOFF', 'FRST', 'RCUR', 'FNAL']):
            raise ValueError("must be one of enum values ('OOFF', 'FRST', 'RCUR', 'FNAL')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of DirectDebitWithAccountDetails from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of DirectDebitWithAccountDetails from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "orders": [DirectDebitOrder.from_dict(_item) for _item in obj["orders"]] if obj.get("orders") is not None else None,
            ...
            "receiver": DirectDebitWithAccountReceiver.from_dict(obj["receiver"]) if obj.get("receiver") is not None else None,
