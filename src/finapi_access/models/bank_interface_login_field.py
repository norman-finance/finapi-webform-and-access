...
class BankInterfaceLoginField(BaseModel):
    """
    Container for a bank's login credential field
    """ # noqa: E501
    label: StrictStr = Field(description="Contains a German label for the input field that you should provide to the user. Also, these labels are used to identify login fields on the API-level, when you pass credentials to the service.")
    is_secret: StrictBool = Field(description="Whether this field has to be treated as a secret. In this case your application should use a password input field instead of a cleartext field.", alias="isSecret")
    is_volatile: StrictBool = Field(description="This field depicts whether the given credential is volatile. If a field is volatile, it means that the value for the field, which is provided by the user, is valid only for a single authentication and then gets invalidated on bank-side. If a login credential field is volatile, it is not possible to store it in finAPI, as stored credentials will never work for future authentications.", alias="isVolatile")
    is_mandatory: StrictBool = Field(description="Indicates whether this is a mandatory or optional login field. We recommend showing all login fields to the users (mandatory and optional).", alias="isMandatory")
    __properties: ClassVar[List[str]] = ["label", "isSecret", "isVolatile", "isMandatory"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of BankInterfaceLoginField from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of BankInterfaceLoginField from a dict"""
        if obj is None:
            return None
