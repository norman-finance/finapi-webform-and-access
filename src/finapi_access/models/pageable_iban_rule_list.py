...
class PageableIbanRuleList(BaseModel):
    """
    Container for IBAN rule information with paging information
    """ # noqa: E501
    iban_rules: List[IbanRule] = Field(description="List of iban rules information<br/> <strong>Type:</strong> IbanRule", alias="ibanRules")
    paging: Paging = Field(description="Information for pagination<br/> <strong>Type:</strong> Paging")
    __properties: ClassVar[List[str]] = ["ibanRules", "paging"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of PageableIbanRuleList from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in iban_rules (list)
        _items = []
        if self.iban_rules:
            for _item in self.iban_rules:
                if _item:
                    _items.append(_item.to_dict())
            _dict['ibanRules'] = _items
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of PageableIbanRuleList from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "ibanRules": [IbanRule.from_dict(_item) for _item in obj["ibanRules"]] if obj.get("ibanRules") is not None else None,
