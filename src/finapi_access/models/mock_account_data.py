...
class MockAccountData(BaseModel):
    """
    Mock account data
    """ # noqa: E501
    account_id: StrictInt = Field(description="Account identifier", alias="accountId")
    account_balance: Union[StrictFloat, StrictInt] = Field(description="The balance that this account should be set to.<br/><br/><b>NOTE</b>:<br/>&bull; If the specified balance does not add up to the account's current balance plus the sum of the 'newTransactions', then finAPI will fix the balance deviation with the insertion of an adjusting entry ('Zwischensaldo' transaction).<br/>&bull; This service is not calculating exchange rates for transactions, so if 'newTransactions' contains any transactions with a currency different to the account's currency, then the balance deviation might get calculated incorrectly.", alias="accountBalance")
    new_transactions: Optional[List[NewTransaction]] = Field(default=None, description="New transactions that should be imported into the account (at most 1000 transactions at once). Please make sure that the value you pass in the 'accountBalance' field equals the current account balance plus the sum of the new transactions that you pass here, otherwise finAPI will detect a deviation in the balance and might add an adjusting entry ('Zwischensaldo' transaction). <br/>Please also note that it is not guaranteed that all transactions that you pass here will actually get imported. More specifically, finAPI will ignore any transactions whose booking date is prior to the date of the last successful account update minus 10 days (which is the default 'update window' that finAPI uses when importing new transactions). Also, finAPI will ignore any transactions that are considered duplicates of already existing transactions within the update window. This is the case for instance when you try to import a new transaction with the same booking date and same amount as an already existing transaction. In such cases, you might get an adjusting entry too ('Zwischensaldo' transaction), as your given balance might not add up to the transactions that will exist in the account after the update.<br/> <strong>Type:</strong> NewTransaction", alias="newTransactions")
    __properties: ClassVar[List[str]] = ["accountId", "accountBalance", "newTransactions"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of MockAccountData from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in new_transactions (list)
        _items = []
        if self.new_transactions:
            for _item in self.new_transactions:
                if _item:
                    _items.append(_item.to_dict())
            _dict['newTransactions'] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of MockAccountData from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "newTransactions": [NewTransaction.from_dict(_item) for _item in obj["newTransactions"]] if obj.get("newTransactions") is not None else None
        })
        return _obj
