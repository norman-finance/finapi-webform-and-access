...
class DailyBalanceList(BaseModel):
    """
    Contains a list of daily balances
    """ # noqa: E501
    latest_common_balance_timestamp: Optional[datetime] = Field(default=None, description="<strong>Format:</strong> 'YYYY-MM-DD'T'HH:MM:SS.SSSXXX' (RFC 3339, section 5.6)<br/>The latestCommonBalanceTimestamp is the latest timestamp at which all regarded  accounts have been up to date. Only balances with their date being smaller than the latestCommonBalanceTimestamp are reliable. <br/>Example: A user has two accounts: A (last update today, so balance from today) and B (last update yesterday, so balance from yesterday). The service /accounts/dailyBalances will return a balance for yesterday and for today, with the info latestCommonBalanceTimestamp=yesterday. Since account B might have received transactions this morning, today's balance might be wrong. So either make sure that all regarded accounts are up to date before calling this service, or use the results carefully in combination with the latestCommonBalanceTimestamp.<br/><br/>NOTE:<br/>The returned balances are completely unreliable if the 'latestCommonBalanceTimestamp' is null. This can occur if the balance download failed for any account or the 'skipBalancesDownload' flag was set to true for the bank connection import or a subsequent update.", alias="latestCommonBalanceTimestamp")
    daily_balances: List[DailyBalance] = Field(description="List of daily balances for the requested period and account(s)<br/> <strong>Type:</strong> DailyBalance", alias="dailyBalances")
    paging: Paging = Field(description="Information for pagination<br/> <strong>Type:</strong> Paging")
    __properties: ClassVar[List[str]] = ["latestCommonBalanceTimestamp", "dailyBalances", "paging"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of DailyBalanceList from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.daily_balances:
            for _item in self.daily_balances:
                if _item:
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of DailyBalanceList from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "latestCommonBalanceTimestamp": obj.get("latestCommonBalanceTimestamp"),
