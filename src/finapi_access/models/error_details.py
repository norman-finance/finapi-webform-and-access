...
class ErrorDetails(BaseModel):
    ...
    code: ErrorCode = Field(description="Error code. See the documentation of the individual services for details about what values may be returned.<br/> <strong>Type:</strong> ErrorCode")
    type: ErrorType = Field(description="Error type. BUSINESS errors depict error messages in the language of the bank (or the preferred language) for the user, e.g. from a bank server. TECHNICAL errors are meant to be read by developers and depict internal errors.<br/> <strong>Type:</strong> ErrorType")
    multi_step_authentication: Optional[MultiStepAuthenticationChallenge] = Field(default=None, description="This field is set when a multi-step authentication is required, i.e. when you need to repeat the original service call and provide additional data. The field contains information about what additional data is required.<br/> <strong>Type:</strong> MultiStepAuthenticationChallenge", alias="multiStepAuthentication")
    __properties: ClassVar[List[str]] = ["message", "code", "type", "multiStepAuthentication"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "code": obj.get("code"),
            "type": obj.get("type"),
            "multiStepAuthentication": MultiStepAuthenticationChallenge.from_dict(obj["multiStepAuthentication"]) if obj.get("multiStepAuthentication") is not None else None
        })
        return _obj
