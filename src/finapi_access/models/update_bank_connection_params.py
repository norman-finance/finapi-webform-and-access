...
class UpdateBankConnectionParams(BaseModel):
    """
    ...
    login_credentials: Optional[List[LoginCredential]] = Field(default=None, description="Set of login credentials. The labels that you pass must match with the login credential labels that the respective interface defines. finAPI will combine the given credentials with any credentials that it has stored. You can leave this field unset in case finAPI has stored all required credentials.<br/> <strong>Type:</strong> LoginCredential", alias="loginCredentials")
    ...
    import_new_accounts: Optional[StrictBool] = Field(default=False, description="Whether to import accounts that weren’t imported previously. Default is false.<br/><br/>NOTES: <br/>&bull; For best performance, you should not enable this flag unless you really expect new accounts to be available in the connection. It is recommended to let your users tell you through your application when they want the service to look for new accounts.<br/>&bull; If you have imported an interface using a limited set of 'accountTypes', you would import all other accounts when setting this flag to true. To avoid importing account types that you are not interested in, make sure to keep this flag unset (or set to false).<br/>&bull; If there is an existing consent and it doesn’t support the import of new accounts (see 'BankConnectionInterface.aisConsent.supportsImportNewAccounts'), then an update attempt with 'importNewAccounts=true' will return an error.", alias="importNewAccounts")
    ...
    load_owner_data: Optional[StrictBool] = Field(default=False, description="Whether to load/refresh information about the bank connection owner(s) - see field 'owners'. Default value is 'false'. Note that owner data is NOT loaded/refreshed during finAPI's automatic bank connection update.", alias="loadOwnerData")
    account_types: Optional[List[AccountType]] = Field(default=None, description="If your client does NOT have any account type restrictions (see <code>ClientConfiguration.accountTypeRestrictions</code>), then this field is only regarded when the field <code>importNewAccounts</code> is set to true, and affects only new accounts that are not yet imported. If the field is specified, then a new account will be imported only when its determined type is one of the given types. If you leave this field unset, then all new accounts will be imported, regardless of their type.<br/><br/><b>NOTE</b>: If your client is restricted to certain account types (see <code>ClientConfiguration.accountTypeRestrictions</code>), then you may only specify account types that are enabled for you, otherwise the service will return with an error. If your client has account type restrictions and you do not specify any types, then the service will implicitly limit the types according to your client's configuration.<br/>Also note that when you have account type restrictions, this field is always regarded independent of the <code>importNewAccounts</code> field, and the restrictions are always applied to ALL accounts (both new accounts and existing accounts).<br/> <strong>Type:</strong> AccountType", alias="accountTypes")
    account_references: Optional[List[AccountReference]] = Field(default=None, description="List of accounts for which access is requested from the bank. It may only be passed if the bank interface has the DETAILED_CONSENT property set. if omitted, finAPI will use the list of existing accounts. Note that the parameter is still required if you want to import new accounts (i.e. call with importNewAccounts=true).<br/> <strong>Type:</strong> AccountReference", alias="accountReferences")
    redirect_url: Optional[Annotated[str, Field(min_length=1, strict=True, max_length=2048)]] = Field(default=None, description="Must only be passed when the used interface has the property REDIRECT_APPROACH. The user will be redirected to the given URL from the bank's website after completing the bank login and (possibly) the SCA.", alias="redirectUrl")
    ...
    __properties: ClassVar[List[str]] = ["bankConnectionId", "bankingInterface", "loginCredentials", "storeSecrets", "importNewAccounts", "skipBalancesDownload", "skipPositionsDownload", "loadOwnerData", "accountTypes", "accountReferences", "redirectUrl", "multiStepAuthentication"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of UpdateBankConnectionParams from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of UpdateBankConnectionParams from a dict"""
        if obj is None:
            return None
