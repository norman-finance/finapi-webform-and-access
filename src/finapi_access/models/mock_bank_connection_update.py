...
class MockBankConnectionUpdate(BaseModel):
    """
    ...
    banking_interface: BankingInterface = Field(description="The banking interface to use.<br/> <strong>Type:</strong> BankingInterface", alias="bankingInterface")
    simulate_bank_login_error: Optional[StrictBool] = Field(default=False, description="Whether to simulate the case that the update fails due to incorrect banking credentials. Note that there is no real communication to any bank server involved, so you won't lock your accounts when enabling this flag. Default value is 'false'.", alias="simulateBankLoginError")
    mock_accounts_data: Optional[List[MockAccountData]] = Field(default=None, description="Mock accounts data. Note that for accounts that exist in a bank connection but that you do not specify in this list, the service will act like those accounts are not received by the bank servers. This means that any accounts that you do not specify here will be marked as deprecated. If you do not specify this list at all, all accounts in the bank connection will be marked as deprecated.<br/> <strong>Type:</strong> MockAccountData", alias="mockAccountsData")
    __properties: ClassVar[List[str]] = ["bankConnectionId", "bankingInterface", "simulateBankLoginError", "mockAccountsData"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of MockBankConnectionUpdate from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in mock_accounts_data (list)
        _items = []
        if self.mock_accounts_data:
            for _item in self.mock_accounts_data:
                if _item:
                    _items.append(_item.to_dict())
            _dict['mockAccountsData'] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of MockBankConnectionUpdate from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "simulateBankLoginError": obj.get("simulateBankLoginError") if obj.get("simulateBankLoginError") is not None else False,
            "mockAccountsData": [MockAccountData.from_dict(_item) for _item in obj["mockAccountsData"]] if obj.get("mockAccountsData") is not None else None
        })
        return _obj
