...
class AccessToken(BaseModel):
    ...
    """ # noqa: E501
    scope: StrictStr = Field(description="Requested scopes (it's always 'all')")
    access_token: StrictStr = Field(description="Access token. Token has a length of up to 8192 characters. Currently tokens with a maximum length of 128 characters are returned, but longer token types may be used in the future.")
    token_type: StrictStr = Field(description="Token type (it's always 'bearer')")
    refresh_token: Optional[StrictStr] = Field(default=None, description="Refresh token. Only set in case of grant_type='password'. Token has a length of up to 8192 characters. Currently tokens with a maximum length of 128 characters are returned, but longer token types may be used in the future.")
    expires_in: StrictInt = Field(description="Expiration time in seconds. A value of 0 means that the token never expires (unless it is explicitly invalidated, e.g. by revocation, or when a user gets locked).")
    __properties: ClassVar[List[str]] = ["scope", "access_token", "token_type", "refresh_token", "expires_in"]
