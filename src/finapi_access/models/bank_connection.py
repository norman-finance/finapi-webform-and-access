...
class BankConnection(BaseModel):
    ...
    """ # noqa: E501
    id: StrictInt = Field(description="Bank connection identifier")
    name: Optional[StrictStr] = Field(default=None, description="Custom name for the bank connection. You can set this field with the 'Edit a bank connection' service, as well as during the initial import of the bank connection.")
    update_status: StrictStr = Field(description="Current status of data download (account balances and transactions/securities). The POST /bankConnections/import and POST /bankConnections/<id>/update services will set this flag to IN_PROGRESS before they return. Once the import or update has finished, the status will be changed to READY.", alias="updateStatus")
    categorization_status: CategorizationStatus = Field(description="Current status of transaction categorization. The asynchronous download process that is triggered by a call of the POST /bankConnections/import and POST /bankConnections/<id>/update services (and also by finAPI's auto update, if enabled) will set this flag to PENDING once the download has finished and a categorization is scheduled for the imported transactions. A separate categorization thread will then start to categorize the transactions (during this process, the status is IN_PROGRESS). When categorization has finished, the status will be (re-)set to READY. Note that the current categorization status should only be queried after the download has finished, i.e. once the download status has switched from IN_PROGRESS to READY.<br/> <strong>Type:</strong> CategorizationStatus", alias="categorizationStatus")
    interfaces: List[BankConnectionInterface] = Field(description="Set of interfaces that are connected for this bank connection.<br/> <strong>Type:</strong> BankConnectionInterface")
    account_ids: List[StrictInt] = Field(description="Identifiers of the accounts that belong to this bank connection", alias="accountIds")
    owners: Optional[List[BankConnectionOwner]] = Field(default=None, description="Information about the owner(s) of the bank connection<br/> <strong>Type:</strong> BankConnectionOwner")
    bank: Bank = Field(description="Bank that this connection belongs to<br/> <strong>Type:</strong> Bank")
    __properties: ClassVar[List[str]] = ["id", "name", "updateStatus", "categorizationStatus", "interfaces", "accountIds", "owners", "bank"]

    @field_validator('update_status')
    def update_status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['IN_PROGRESS', 'READY']):
            raise ValueError("must be one of enum values ('IN_PROGRESS', 'READY')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of BankConnection from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of BankConnection from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "categorizationStatus": obj.get("categorizationStatus"),
            "interfaces": [BankConnectionInterface.from_dict(_item) for _item in obj["interfaces"]] if obj.get("interfaces") is not None else None,
            "accountIds": obj.get("accountIds"),
            "owners": [BankConnectionOwner.from_dict(_item) for _item in obj["owners"]] if obj.get("owners") is not None else None,
